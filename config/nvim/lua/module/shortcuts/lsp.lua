	...
	data = function()
		return {
			on_attach = function(client, bufnr)
				...
				shortcut.map("n", "<space>h", vim.lsp.buf.signature_help, options)
				...
				shortcut.map("i", "<F2>", vim.lsp.buf.rename, options)
				shortcut.map("n", "<C-k>", vim.lsp.buf.code_action, options)
				shortcut.map("i", "<C-k>", vim.lsp.buf.code_action, options)
